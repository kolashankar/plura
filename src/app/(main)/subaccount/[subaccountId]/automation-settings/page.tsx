...
import { Settings, Play, Pause, ExternalLink, AlertCircle, CheckCircle2, Zap, Mail, Users, Globe, Lock } from 'lucide-react'
...
export default function AutomationSettingsPage() {
  ...
  const fetchAvailableForms = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/automation-settings/available?subaccountId=${subaccountId}&userType=agency`)
      if (response.ok) {
        const data = await response.json()
        setForms(data.forms || [])
      } else if (response.status === 403) {
        // User restriction - show error message
        const data = await response.json()
        console.log('Access restricted:', data.message)
      }
    } catch (error) {
  ...
  const getAccessBadge = (form: AutomationForm) => {
    ...
    if (form.restrictToAgency) {
      return <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200">Agency</Badge>
    }
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-300 mt-2">
            Access automation forms and workflows available to your account
          </p>
        </div>
      </div>
      ...
      {forms.length === 0 && (
        <Alert>
          ...
          <AlertDescription>
            No automation forms are currently available for your account. Contact your administrator to access automation workflows.
          </AlertDescription>
        </Alert>
      )}
