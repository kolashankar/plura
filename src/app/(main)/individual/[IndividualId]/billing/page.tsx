...
const page = async ({ params }: Props) => {
  ...
  const currentPlanDetails = pricingCards.find(
    (c) => c.priceId === individualSubscription?.Subscription?.priceId
  )

  const charges = await stripe.charges.list({
    limit: 50,
    customer: individualSubscription?.customerId,
  })
  ...
  return (
    <>
      <SubscriptionHelper
        prices={prices.data}
        customerId={individualSubscription?.customerId || ''}
        planExists={individualSubscription?.Subscription?.active === true}
      />
      ...
      <div className="flex flex-col lg:!flex-row justify-between gap-8">
        <PricingCard
          ...
          customerId={individualSubscription?.customerId || ''}
          amt={
            individualSubscription?.Subscription?.active === true
